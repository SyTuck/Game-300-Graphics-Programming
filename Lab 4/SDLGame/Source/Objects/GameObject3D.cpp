...
int GameObject3D::CheckCollision(GameObject3D otherObj)
{
	...
	if ((LocalPos.x - (dimensions.x*1.75) + velocity.x < OtherPos.x - otherObj.dimensions.x) ||		//reversed the +/- on LocalPos so Left Most edge of this object
		(LocalPos.x + (dimensions.x*1.75) + velocity.x > OtherPos.x + otherObj.dimensions.x))		//is checked agains the Right Most of the other object
	...
	if ((LocalPos.y - (dimensions.y*1.75) + velocity.y < OtherPos.y - otherObj.dimensions.y) ||		//modifying the bounding box of the pumpkin because I don't think it's sized right
		(LocalPos.y + (dimensions.y*1.75) + velocity.y > OtherPos.y + otherObj.dimensions.y))		//(and it's very apparent now that I'm doing a "Is within" check)
	{
		retAxis |= YCOLLISION;
	}
	if ((LocalPos.z - (dimensions.z*1.75) + velocity.z < OtherPos.z - otherObj.dimensions.z) ||		//(note that the Velocity is only ever added because the sign is already included in it's value)
		(LocalPos.z + (dimensions.z*1.75) + velocity.z > OtherPos.z + otherObj.dimensions.z))
	{
		retAxis |= ZCOLLISION;
	}
